...
# 雷達圖報酬軸專用的 beta 滿分參考（與 PREF_BETA_REF 解耦，避免動到已驗證的 win_VT）。
# 本最佳化系統的投組 beta 天花板約 1.1~1.2，故 REF=2.0 會把報酬軸壓在 0.5~0.56 失去鑑別度；
# 改用 1.2 後 β=1.11→0.78（明顯偏高、仍留頂端空間）、β=1.2 以上→1.0。市場 β=1→0.5、低於市場 floor 0.5。
RADAR_BETA_REF = 1.2
