...
## 2026-06-06：metrics_comparison 圖改版（CAGR 併入累積報酬、加平均費用率）+ 兩問查核

### 圖表（使用者要求）
- `_plot_backtest_metrics_comparison`：① 累積總報酬長條的每根上方**加註年化 CAGR**（如「118 (年化11.9%)」）；② **刪除原「年化報酬率 CAGR」子圖**（與累積報酬重複），改放**「平均費用率 %（每期加權，越低越好）」**（取 `dimension_comparison` 的 `Avg_Raw_Expense_Ratio_%`）；③ 殖利率/波動/夏普/最大回撤維持。suptitle 移除寫死的「季度再平衡」（頻率會變）。7 張 showcase 已重生驗證。

### 查核（回應使用者兩問，皆非 bug）
- **Q1：低成本勝率時而高、本次卻 0%** —— 根因是 g(w) 的**傾斜強度 τ = 0.30·(1−T_growth)·R**，其中 **R = 報酬兩維權重(CAGR+股息)**。本次使用者把報酬權重幾乎設 0 → R=0.015 → **τ=0.004（傾斜≈關閉）** → 系統幾乎只做純 minvar（挑低波動 ETF：USMV/HDV/XLP…），**沒有往「低成本」傾斜** → 費用率輸 VT。對照 `cost_liquidity` 原型 R=0.28 → τ=0.044（10×）→ 強力往成本/流動性傾斜 → 贏。**＝設計特性（非 bug）**：報酬無感的使用者，其「成本/分散/流動性」偏好幾乎不被傾斜表達。若要改善需動 g(w)（讓 τ 也吃非報酬偏好質量），屬最佳化器改動、需另行 A/B + 批准，本次**未動**、僅記錄。
- **Q2：MaxDD 變了、VT 也變** —— VT MaxDD 非固定常數，是「**本次回測視窗**」內量的。兩次再平衡頻率不同（income 季度起點 2019-07-01；本次年度起點 2019-12-31）→ 視窗/再平衡格點不同 → VT 回撤 −33.72% vs −34.00%、系統投組與視窗亦不同。各自正確。

## 2026-06-06：回測雷達報酬軸改 beta + 網頁停用「早停」（答完 9 題前不可結束）

- **#1 雷達報酬軸用 beta**：`_plot_backtest_preference_radar` 的「報酬」軸由 `CAGR_%` 改為 `Avg_Score_Return_CAGR`（即偏好評分用的 beta 對 VT 報酬子分數），與系統「報酬維度＝系統性風險曝險(β)」一致。標籤改「報酬 (β對VT)」。7 張 showcase 重生。
- **#2 停用早停**：使用者反映未答完 9 題會壓低未問維度（CI 不可信）。`etf_web/app.py._compute_next` 移除覆蓋階段的 T1 早停提議；`app.js` 移除問題卡的「結束問答」鈕、改提示「將完整詢問 9 個面向」。引擎一律問到 9 維全覆蓋；T2（9 題後的最後重問確認）保留（屬答完 9 題後，非早停）。
- **#3 τ 公式為何含報酬係數 R（回答使用者，未改碼）**：見下方說明；R 把「願意偏離風險核心的程度」綁在報酬胃納上，對「重成本/分散但不在意報酬」的使用者會使 τ≈0，屬已知設計取捨，若要改需 A/B+批准。

## 2026-06-06：範例偏好一鍵帶入 + 打包成「應用程式」(icon/exe/捷徑/zip)

- **範例偏好一鍵帶入**（前端）：第一頁加 3 顆按鈕（現金流/收入、成長/報酬、保守/抗跌）；按下會帶入該理念並**自動逐題回答 9 題**（依 dim_key 給示範答案），驅動真引擎產出權重，直接到 done、可前往執行分析。現金流(income) 偏好強調股息。
- **應用程式打包 `app/`**（非程式邏輯）：
  - `etf_icon.ico`/`.png`：PIL 產生的圖示（藍底＋ETF＋上升長條）。
  - `啟動_ETF.bat`：雙擊即跑 `etf_web/run_web.py`（最簡單）。
  - `_launcher_bootstrap.py`＋`build_exe.py`：PyInstaller 建「小型啟動器 exe」（`ETF偏好投組.exe`，~6MB，帶圖示；執行時用系統 Python 跑 run_web，故小、可隨整包傳）。已實測建置成功。
  - `建立桌面捷徑.ps1`：桌面建帶圖示捷徑（優先指向 app/exe→dist/exe→bat）。
  - `打包成zip.ps1`：robocopy 排除 .git/__pycache__/encoder_model/dist/log → 壓成可傳 zip（模型對方首次執行自動下載）。
