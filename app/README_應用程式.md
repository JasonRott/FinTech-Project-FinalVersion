...
## API 金鑰（別人怎麼輸入）
本系統用到的金鑰：`AV_API_KEY`（Alpha Vantage 行情特徵）、`FINNHUB_API_KEY`（新聞/情緒）、`GEMINI_API_KEY`（只有 Gemini 訪談模式才用）。
- **用內附的行情/情緒快取做展示時，三把都可不填**（系統會走快取 + 免金鑰的 yfinance）。要抓「最新／更多」資料才需要金鑰。
- **怎麼填**：把 `.env.example` 複製成 **`.env`**，放在 **exe 同一個資料夾**（單機版）或專案根（.bat 版），填入金鑰即可。程式啟動會自動讀取。
  - 單機版：`.env` 放在 `ETF偏好投組\` 資料夾裡（和 `ETF偏好投組.exe` 同層）。
- `.env` 含機密、已被 `.gitignore` 排除；發佈給別人時**不要**附上你自己的 `.env`（附 `.env.example` 範本即可，讓對方填自己的金鑰）。
