...
一套**偏好驅動、且有 Black-Litterman / CAPM 理論地基**的 ETF robo-advisor:使用者偏好 → 自動決定投組「核心類型 + 風險水準 + 偏好傾斜」。經跨市場規制的 walk-forward 驗證:**報酬導向使用者穩健取得高於 VT 的絕對報酬;保守/收入型贏在風險效率與偏好滿足——全部 7 種使用者原型的投組在樣本外偏好分數上都勝過 VT(win_VT 89–100%）、亦全面勝過等權與最大夏普。** 系統對自身限制誠實量化。
...
> ⚠️ **註（評分修正範圍，2026-06-06）**：以下 §3.2 / §3.3 / §3.4 的 **win_VT 數字為「MaxDD 評分修正前」的舊尺度**（當時單一標的基準 VT 在抗跌維度被退化評分強制給滿分 1.0，且各策略各用自身持股當尺度），這些 A/B 方法學實驗**未重跑**。由於該瑕疵對「VT 的分數」在各 arm 比較中是**一致性地灌水**，這些段落的**方向性結論**（beta 評分優於過去 CAGR、統一 BL 較差、硬品質約束無益）仍成立，且各自另有**未受污染的證據**支撐（V-1 顯示過去 CAGR 對未來報酬 r≈0；BL/品質約束差異源自投組權重行為，非評分）。**修正後的生產級 win_VT 以 §3.6（七使用者）與 showcase 對照報告為準**（修正後全員 89–100%）。
...
10. **偏好傾斜 τ 與「報酬胃納」解耦**：目前 g(w) 的傾斜強度 `τ = TAU_MAP_COEF·(1−T_growth)·R`,其中 `R＝報酬兩維(資本增值+股息)權重`。原意是讓「想要報酬但防禦」的收入型自動拿到較大 τ 做股息傾斜;但副作用是——對「重成本/分散、卻不在意報酬」的使用者,R≈0 → τ≈0 → 系統退回近乎純 minvar,**未充分表達其非報酬偏好(費用率/分散/流動性)**。未來可將 R 改為「**非核心偏好質量**」(未被核心選擇用到的維度權重總和)或給 τ 一個下限,使任何強烈偏好都能被傾斜表達。因傾斜效果**強烈 profile 依賴**,須 walk-forward A/B 後再採用。(註:在**九題全覆蓋**、各維權重較平衡時此問題較輕;極端單一偏好時較明顯。)

---

## 6. 結論
本系統的價值在於:**理論地基紮實(BL/CAPM)、偏好驅動、profile-dependent、跨規制驗證、且對限制誠實**。它能讓報酬導向使用者穩健取得 >VT 的絕對報酬,並讓各類使用者的偏好在樣本外可被**驗證地**滿足(7 種原型投組的偏好分數對 VT 勝率 89–100%、對等權/最大夏普亦全面領先);但不應宣稱為穩定 alpha。作為決策支援框架與研究成果,結論站得住腳。
